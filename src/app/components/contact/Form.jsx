...
const Form = () =>  {
  ...
  const sendEmail = (params) => {
    ...
    const toastId = toast.loading("Sending your message, please wait..., please wait");
  ...
  return (
    ...
    >
      ...
      variants={item} 
      type="email" 
      placeholder="email" {...register("email", {required: "This field is required", pattern: /^\S+@\S+$/i})}
      className='w-full p-2 rounded-md shadow-lg text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 custom-bg'
       />
         {
...
/>
      ...
      <motion.input  
      variants={item}
      value="Invoke contact" 
      type="submit" 
      className="mt-10 px-10 py-4 rounded-md shadow-lg bg-background border border-accent/30 border-solid
